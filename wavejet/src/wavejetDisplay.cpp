...
void WavejetDisplay::display (Viz3d& cam, 
                              ...
                              u_int idx)
{
    //cam.showWidget("points", WCloud(_f, Color::black()));
    display_1st_radius(cam, transform, radius_scale, idx);
    display_all_radius(cam, transform, radius_scale, idx);
}

void WavejetDisplay::display(cv::viz::Viz3d& cam, u_int idx)
{
    display(cam, cv::Affine3d(), 1, idx);
}
                                         ...
                                         u_int idx)
{
    ...
    { // draw the first radius
        ...
        vector<Point3d> triangle { _f.at(0) * radius_scale,
                                   _f.at(i) * radius_scale, 
                                   _f.at(j) * radius_scale};
                                        ...
                                        u_int idx)
{
    ...
    { // draw other radius
        for (u_int t = ANGLE_DIVISION; t < ANGLE_DIVISION*2; ++t)
        {
            ...
            //cout << i << ", " << j << endl;
            vector<Point3d> paral { _f.at(i) * radius_scale, 
                                    _f.at(j) * radius_scale, 
                                    _f.at(i - ANGLE_DIVISION) * radius_scale,
                                    _f.at(j - ANGLE_DIVISION) * radius_scale };
...
void WavejetDisplay::compute_and_find_point_position()
{
    ...
    //cout << _f.size() << ", " << ANGLE_DIVISION * (RADIUS_DIVISION-1) + 1 << endl;
    assert(_f.size() == ANGLE_DIVISION * (RADIUS_DIVISION-1) + 1);
}
